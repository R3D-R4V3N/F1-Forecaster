...
def engineer_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["driver_recent_avg"] = (
        df.groupby("driver")["position"].apply(lambda s: s.shift().rolling(window=5, min_periods=1).mean())
    )

    # Average finish at this circuit for each driver
    df["driver_circuit_avg"] = (
        df.groupby(["driver", "circuitId"])["position"].apply(lambda s: s.shift().expanding().mean())
    )

    # Average pit stop time per team recent races
    df["team_recent_pit_avg"] = (
        df.groupby("constructor")["pit_stop_time"].apply(lambda s: s.shift().rolling(window=3, min_periods=1).mean())
    )
